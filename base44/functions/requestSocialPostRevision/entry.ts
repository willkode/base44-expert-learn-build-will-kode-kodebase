...
Deno.serve(async (req) => {
  try {
    ...
    const { social_post_id, revision_notes } = await req.json();
    ...
    try {
      await base44.entities.SocialAutomationLog.create({
        ...
        status: "warning",
        message: `Revision requested on "${post.title_internal || social_post_id}" by ${user.email}.`,
        metadata: { from_status: post.approval_status, to_status: "revision_requested", revision_notes: revision_notes || "" },
      });
