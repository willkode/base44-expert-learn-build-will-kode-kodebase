...
export default function CartSheet() {
  const { items, removeItem, isOpen, setIsOpen, closeCart } = useCart();
  ...
  const totalCents = cartProducts.reduce((sum, p) => sum + getProductSalePriceCents(p.priceCents), 0);
                ...
                <div key={p.id} className="flex gap-3 rounded-xl border border-border bg-card/60 p-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-1.5 mt-1">
                      <span className="font-sora font-bold text-sm">{formatUsd(getProductSalePriceCents(p.priceCents))}</span>
                      {onSale && (
            ...
            <div className="border-t border-border pt-4 space-y-3">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="flex items-center gap-2">
                  {onSale && fullTotalCents > totalCents && (
