...
export default function MigrationNext() {
  const [form, setForm] = useState({ name: "", email: "", appUrl: "", stack: "", notes: "" });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await base44.functions.invoke("submitContactForm", {
        name: form.name,
        email: form.email,
        message: `BASE44 MIGRATION — new order ($199)\n\nApp URL: ${form.appUrl || "Not provided"}\n\nPreferred stack / hosting: ${form.stack || "No preference"}\n\nNotes: ${form.notes || "None"}`,
      });
      trackEvent("migration_details_submit", {});
      setSent(true);
    } catch {
  ...
  return (
    <>
      <Seo
        title="Base44 Migration — Next Steps | KodeBase"
        description="Your Base44 migration is confirmed. Send your app details and add Will as a collaborator."
        path="/services/base44-migration/next"
        noindex
      />
        ...
        <div className="max-w-lg mx-auto px-6">
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground text-sm">
              Send your app details, then add me as a collaborator so I can start the migration.
            </p>
          </div>

          {/* Step 1 — details form */}
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl border border-border bg-card p-8 mb-6">
            <p className="text-xs font-semibold text-primary uppercase tracking-wider mb-4">Step 1 — Your app details</p>
            {sent ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium mb-1.5">
                    Preferred stack or hosting <span className="text-muted-foreground font-normal">(optional)</span>
                  </label>
                  <input name="stack" value={form.stack} onChange={handleChange} placeholder="Supabase + Vercel, AWS, no preference…"
                    className="w-full rounded-lg border border-input bg-background px-4 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-ring placeholder:text-muted-foreground" />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1.5">
                    Anything I should know about the app? <span className="text-muted-foreground font-normal">(optional)</span>
                  </label>
                  <textarea name="notes" value={form.notes} onChange={handleChange} rows={3}
                    placeholder="Payments, integrations, AI agents, data volume, deadlines…"
          ...
          <div className="rounded-2xl border-2 border-primary/50 bg-primary/5 p-8">
            ...
            <div className="flex items-start gap-3 mb-5">
              ...
              <p className="text-sm text-foreground/90 leading-relaxed">
                The migration can't start until I have access to your app. Add me as a collaborator in Base44 so I can
                review your entities, functions and integrations.
              </p>
            </div>
            ...
            <div className="space-y-3">
              {[
                ...
                "I'll accept, scope the migration and email you a start date.",
              ].map((text, i) => (
